...
const { getPerformanceReport } = require('./alert_ttr_indihome');
...
(async () => {
  ...
  try {
    ...
    const targets = [
      {
        url: 'https://sympony.tif3.net/bot-assurance/indihome',
        sel: '#captureArea',
        name: 'alert-indihome.png',
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        const captionText = `Mohon Izin <b>Bapak/Ibu: ${namaPenerima}</b>\n` + `Report ${title}\n` + `Posisi : ${cutoffTime}\n\n` + `<code>${reportData.summaryText}</code>`;
